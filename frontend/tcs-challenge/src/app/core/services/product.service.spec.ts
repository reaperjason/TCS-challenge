...
describe('ProductService', () => {
  ...
  it('should fetch all products with minimal structure', () => {
    const dummyApiData: ProductsApiResponse<Product[]> = {
      data: [
        { id: '1', name: 'Prod 1', description: 'desc', logo: 'logo1.png' } as Product,
        { id: '2', name: 'Prod 2', description: 'desc', logo: 'logo2.png' } as Product,
      ],
    };
  ...
  it('should update a product via PUT request and return the updated product data', () => {
    ...
    service.updateProduct(productId, productToUpdate).subscribe(response => {
      expect(response).toHaveProperty('id', 'uno');
      expect(response).toHaveProperty('name', 'visa silver');
    });
    const request = httpMock.expectOne(`http://localhost:3002/bp/products/${productId}`);
